...
def normal_scan():
    ...
    if folder_selected:
        ...
        for file in real_files:
            file_path = os.path.join(folder_selected, file)  # 完整的文件路径
            ...
            # 检查MD5哈希值是否在危险列表中
            if md5 in aaa:
                danger_count += 1  # 增加危险文件计数器
                danger_files.append(file_path)  # 将危险文件路径添加到列表中
                # 打印文件名（这里打印的是绝对路径）
                print(file_path)

        # 更新Tkinter组件以显示危险文件数量
        wf.config(text=f'共发现{danger_count}个病毒')

        # 如果发现了危险文件，配置删除按钮
        if danger_count > 0:
            print(f"Number of dangerous files: {danger_count}")
            # 配置删除按钮，使其在点击时调用dele函数并传递危险文件列表
            de.config(command=lambda: dele(danger_files))
        else:
            de.config(state=tk.DISABLED)

def quick_scan():
    def delete_files_with_keywords(folder_path, aaa):
        count = 0
        for root, dirs, files in os.walk(folder_path):
            for file_name in files:
                if any(keyword in file_name for keyword in aaa):
    ...
    def select_folder_and_delete_files():
        ...
        if folder_path:
            ...
            total_deleted = delete_files_with_keywords(folder_path, keywords)
            wf.config(text = f'共发现{total_deleted}个病毒')
...
file = ['滑稽病毒', '熊猫烧香', '金猪报喜', 'Error', '魔理沙', '毁灭', '勒索']
...
eedd = Button(root, text = '快速扫描', command = quick_scan)
